...
""" % (prereq, q, q, prereq))
      ...
      try:
        ...
        #TimeoutFunction(getattr(gradingModule, q),1200)(self) # Call the question's function͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
""")
  ...
  def addErrorHints(self, exceptionMap, errorInstance, questionNum):
    ...
    # question specific error hints͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    if exceptionMap.get(questionName):
      questionMap = exceptionMap.get(questionName)
      if (questionMap.get(typeOf)):
        errorHint = questionMap.get(typeOf)
    # fall back to general error messages if a question specific͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    # one does not exist͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    if (exceptionMap.get(typeOf)):
      errorHint = exceptionMap.get(typeOf)

    # dont include the HTML if we have no error hint͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    if not errorHint:
      return ''
  ...
  def produceGradeScopeOutput(self):
    ...
    # individual tests͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    tests_out = []
    for name in self.questions:
      test_out = {}
      # test name͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
      test_out['name'] = name
      # test score͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
      test_out['score'] = self.points[name]
      test_out['max_score'] = self.maxes[name]
      # others͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
  ...
  def produceOutput(self):
    ...
    for q in self.questions:
      ...
      if (self.points[q] >= self.maxes[q]):
        checkOrX = '<span class="correct"/>'
      #messages = '\n<br/>\n'.join(self.messages[q])͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
  ...
  def addMessage(self, message, raw=False):
    if not raw:
        # We assume raw messages, formatted for HTML, are printed separately͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        if self.mute: util.unmutePrint()
        print('*** ' + message)
        if self.mute: util.mutePrint()
  ...
  def addMessageToEmail(self, message):
    ...
    for line in message.split('\n'):
      pass
      #print '%%% ' + line + ' %%%'
      #self.messages[self.currentQuestion].append(line)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

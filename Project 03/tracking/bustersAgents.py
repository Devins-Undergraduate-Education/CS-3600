# bustersAgents.py͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
# ----------------͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # loop through ghost positions and find the closest ghost 
        for ghostDist in livingGhostPositionDistributions:

            # calculate dist to ghost            
            distToGhost = self.distancer.getDistance(pacmanPosition, ghostDist.argMax())

            # check for new closer ghost
            if distToGhost < minGhostDist:
                minGhostDist = distToGhost
                minGhostPos = ghostDist.argMax()

        # initialize ghost values
        minGhostDist = float('inf')
        bestAction = None

        # find best move based on curr ghost location
        for move in legal:

            # calculate best action, checking dist aver move
            distanceAfterAction = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, move), minGhostPos)

            # check for new better move
            if distanceAfterAction < minGhostDist:
                minGhostDist = distanceAfterAction
                bestAction = move
        
        # return best move to catch them ghosts!
        return bestAction
...
class Perceptron(object):
    ...
    def __init__(self, inSize=1, weights=None):
        ...
        if weights is None:
            #weights of previous layers into this one, random if passed in as None͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    ...
    def sigmoidActivation(self, inActs):
        ...
        # iterate through inActs and add to biasedInput array
        for entry in inActs:
            biasedInput.append(entry)

        # return based on bias input and inActs
        return self.sigmoid(self.getWeightedSum(biasedInput))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for _ in range(len(biasedInput)):
            # calculate update weight 
            prevWeight = self.weights[entry]
            updatedWeight = biasedInput[entry] * alpha * delta + prevWeight

            # update totalMod value and weights
            totalModification += abs(updatedWeight - prevWeight)
            self.weights[entry] = updatedWeight

            # increment to add to next entry 
            entry += 1
...
class NeuralNet(object):
    ...
    def __init__(self, layerSize):#default 3 layer, 1 percep per layer
        ...
        #build hidden layer(s)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        ...
        #build output layer͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        for i in range(layerSize[-1]):
            percep = Perceptron(layerSize[-2]) # num of perceps feeding into this one
            self.outputLayer.append(percep)

        #build layers list that holds all layers in order - use this structure͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        # to implement back propagation͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    ...
    def feedForward(self, inActs):
        ...
        # intialize list to store input and output data
        processedData = [inActs]
        recentData = inActs

        # iterate through each layer
        for layer in self.layers:
            processedPerceptions = [] # initalize a list to store processed perceptions

            # iterate through each perception
            for perception in layer:
                processedPerceptions.append(perception.sigmoidActivation(recentData))

            # add processed data to the list and set newest data variable
            processedData.append(processedPerceptions)
            recentData = processedPerceptions
    ...
    def backPropLearning(self, examples, alpha):
        ...
        """
        #keep track of output͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        averageError = 0
        averageWeightChange = 0
        numWeights = 0

        for example in examples:#for each example
            #keep track of deltas to use in weight change͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            deltas = []
            #Neural net output list͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            allLayerOutput = self.feedForward(example[0])
            lastLayerOutput = allLayerOutput[-1]
            #Empty output layer delta list͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            outDelta = []
            #iterate through all output layer neurons͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                hiddenDelta = []
                #Iterate through all neurons in this layer͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            ...
            #end for each example͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #calculate final output͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    """
        ...
        #else :͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    print '.',͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

    # loop through, updating until reach modification weight threshold
    while not iteration > maxItr and not weightMod < weightChangeThreshold:
        # update variables (DO NOT DELETE. BAD TINGS HAPPEN)
        trainError, weightMod = nnet.backPropLearning(examplesTrain, alpha)
        iteration += 1

        # funky code given to us
        if iteration % 10 == 0:
            print ('! on iteration %d; training error %f and weight change %f'%(iteration, trainError, weightMod))
        else: 
            print ('.')
    ...
    # iterate through data in test set
    for dataInput, dataOutput in examplesTest:
        # get neural net output
        neuralOut = nnet.feedForward(dataInput)[-1]
        mismatch = False # initalize flag

        # check for mismatch
        for index in range(len(neuralOut)):
            if round(neuralOut[index]) != dataOutput[index]:
                mismatch = True

#utility functions for neural net project͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
import random
...
def getNNXorData(fileString ="datasets/xor.txt", limit=100000 ):
    """
    returns limit # of examples from file passed as string
    """
    examples=[]
...
def buildExamplesFromCarData(size=200):
    ...
    for cdRec in carData:
        ...
        for cdInRec in cdRec[0] :
            for val in cdInRec :
                tmpInVec.append(val)
        #print "in :" + str(cdRec) + " in vec : " + str(tmpInVec)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        tmpList = (tmpInVec, cdRec[1])
        carDataTrainList.append(tmpList)
    #print "car data list : " + str(carDataList)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

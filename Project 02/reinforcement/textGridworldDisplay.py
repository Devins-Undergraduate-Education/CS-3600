# textGridworldDisplay.py͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
# -----------------------͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
def prettyPrintNullValues(gridWorld, currentState = None):
    ...
    for y in range(grid.height):
        ...
        for x in range(grid.width):
            ...
            # value = values[state]͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

            action = None
            # if policy != None and state in policy:͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            #   action = policy[state]͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            #
            actions = gridWorld.getPossibleActions(state)
            ...
            valString = None
            # if action == 'exit':͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            #   valString = border('%.2f' % value)
            # else:͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            #   valString = '\n\n%.2f\n\n' % value
            #   valString += ' '*maxLen͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
# INDENTING CODE͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

# Indenting code based on a post from George Sakkis͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
# (http://aspn.activestate.com/ASPN/Cookbook/Python/Recipe/267662)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
def indent(rows, hasHeader=False, headerChar='-', delim=' | ', justify='left',
    ...
    # closure for breaking logical rows to physical, using wrapfunc͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    def rowWrapper(row):
        newRows = [wrapfunc(item).split('\n') for item in row]
        return [[substr or '' for substr in item] for item in list(*newRows)]
    # break each logical row into one or more physical ones͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    logicalRows = [rowWrapper(row) for row in rows]
    # columns of physical rows͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    columns = list(*reduce(operator.add,logicalRows))
    # get the maximum of each column by the string length of its items͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    maxWidths = [max([len(str(item)) for item in column]) for column in columns]
    rowSeparator = headerChar * (len(prefix) + len(postfix) + sum(maxWidths) + \
                                 len(delim)*(len(maxWidths)-1))
    # select the appropriate justify method͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
# TEST OF DISPLAY CODE͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

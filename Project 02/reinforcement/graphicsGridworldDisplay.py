# graphicsGridworldDisplay.py͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
# ---------------------------͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
class GraphicsGridworldDisplay:
    ...
    def displayNullValues(self, currentState = None, message = ''):
        values = util.Counter()
        #policy = {}͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        states = self.gridworld.getStates()
        for state in states:
            values[state] = 0.0
            #policy[state] = agent.getPolicy(state)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        drawNullValues(self.gridworld, currentState,'')
        # drawValues(self.gridworld, values, policy, currentState, message)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        sleep(0.05 / self.speed)
...
def drawNullSquare(grid,x, y, isObstacle, isTerminal, isCurrent):
    ...
    # if not isObstacle:͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    #   text( (screen_x, screen_y), text_color, valStr, "Courier", 24, "bold", "c")͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
def drawSquareQ(x, y, qVals, minVal, maxVal, valStrs, bestActions, isCurrent):
    ...
    for action in actions:
        ...
        if action == 'north':
            ...
            #text(n, text_color, valStr, "Courier", 8, "bold", "n")͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        if action == 'south':
            polygon( (center, sw, se), wedge_color, filled = 1, smoothed = False)
            #text(s, text_color, valStr, "Courier", 8, "bold", "s")͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        if action == 'east':
            polygon( (center, ne, se), wedge_color, filled = 1, smoothed = False)
            #text(e, text_color, valStr, "Courier", 8, "bold", "e")͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        if action == 'west':
            polygon( (center, nw, sw), wedge_color, filled = 1, smoothed = False)
            #text(w, text_color, valStr, "Courier", 8, "bold", "w")͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    ...
    for action in actions:
        ...
        h = -20
        if action == 'north':
            #polygon( (center, nw, ne), wedge_color, filled = 1, smooth = 0)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            text(n, text_color, valStr, "Courier", h, "bold", "n")
        if action == 'south':
            #polygon( (center, sw, se), wedge_color, filled = 1, smooth = 0)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            text(s, text_color, valStr, "Courier", h, "bold", "s")
        if action == 'east':
            #polygon( (center, ne, se), wedge_color, filled = 1, smooth = 0)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            text(e, text_color, valStr, "Courier", h, "bold", "e")
        if action == 'west':
            #polygon( (center, nw, sw), wedge_color, filled = 1, smooth = 0)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

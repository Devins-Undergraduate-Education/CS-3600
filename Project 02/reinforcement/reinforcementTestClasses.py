# reinforcementTestClasses.py͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
# ---------------------------͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    ...
    def comparePrettyValues(self, aPretty, bPretty, tolerance=0.01):
        ...
        for a, b in zip(aList, bList):
            try:
                aNum = float(a)
                bNum = float(b)
                # error = abs((aNum - bNum) / ((aNum + bNum) / 2.0))͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
                error = abs(aNum - bNum)
                if error > tolerance:
                    return False
            except ValueError:
...
class ApproximateQLearningTest(testClasses.TestCase):
    ...
    def runAgent(self, moduleDict, numExperiences):
        ...
        # choose a random start state and a random possible action from that state͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        # get the next state and reward from the transition function͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        lastExperience = None
    ...
    def testEpsilonGreedy(self, moduleDict, tolerance=0.025):
        ...
        for state in self.grid.getStates():
            ...
            for iteration in range(self.numIterations):
                # assume that their computeActionFromQValues implementation is correct (q4 tests this)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
                if agent.getAction(state) == optimalAction:
                    numGreedyChoices += 1
            # e = epsilon, g = # greedy actions, n = numIterations, k = numLegalActions͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            # g = n * [(1-e) + e/k] -> e = (n - g) / (n - n/k)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
### q5/q6͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
### =====͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
## Average wins of a pacman agent͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict):
        ...
        for passed, points, value, minimum, thresholds, name in results:
            ...
            # print passed, points, value, minimum, thresholds, name͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            totalPoints += points
            if not passed:
                assert points == 0
...
### q2/q3͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
### =====͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
## For each parameter setting, compute the optimal policy, see if it satisfies some properties͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
class GridPolicyTest(testClasses.TestCase):
    ...
    def __init__(self, question, testDict):
        ...
        # Function in module in analysis that returns (discount, noise)͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        self.parameterFn = testDict['parameterFn']
        self.question2 = testDict.get('question2', 'false').lower() == 'true'

        # GridWorld specification͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    _ is empty space͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    numbers are terminal states with that value͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    # is a wall͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    S is a start state͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #
        self.gridText = testDict['grid']
        self.grid = gridworld.Gridworld(parseGrid(testDict['grid']))
        self.gridName = testDict['gridName']

        # Policy specification͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    _                  policy choice not checked͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    N, E, S, W policy action must be north, east, south, west͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #
        self.policy = parseGrid(testDict['policy'])

        # State the most probable path must visit͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    (x,y) for a particular location; (0,0) is bottom left͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    terminal for the terminal state͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        self.pathVisits = testDict.get('pathVisits', None)

        # State the most probable path must not visit͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    (x,y) for a particular location; (0,0) is bottom left͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #    terminal for the terminal state͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    ...
    def printPolicy(self, policy, policyTypeIsGrid):
        ...
        # for state in sorted(self.grid.getStates()):͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        #     if state != 'TERMINAL_STATE':͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅

# featureExtractors.py͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
# --------------------͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
...
def closestFood(pos, food, walls):
    ...
    while fringe:
        ...
        # if we find a food at this location then exit͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        if food[pos_x][pos_y]:
            return dist
        # otherwise spread out from the location to its neighbours͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        nbrs = Actions.getLegalNeighbors((pos_x, pos_y), walls)
        for nbr_x, nbr_y in nbrs:
            fringe.append((nbr_x, nbr_y, dist+1))
    # no food found͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
    return None
...
class SimpleExtractor(FeatureExtractor):
    ...
    def getFeatures(self, state, action):
        # extract the grid of food and wall locations and get the ghost locations͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        food = state.getFood()
        ...
        # compute the location of pacman after he takes the action͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        x, y = state.getPacmanPosition()
        dx, dy = Actions.directionToVector(action)
        next_x, next_y = int(x + dx), int(y + dy)

        # count the number of ghosts 1-step away͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        features["#-of-ghosts-1-step-away"] = sum((next_x, next_y) in Actions.getLegalNeighbors(g, walls) for g in ghosts)

        # if there is no danger of ghosts then add the food feature͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
        if not features["#-of-ghosts-1-step-away"] and food[next_x][next_y]:
            features["eats-food"] = 1.0

        dist = closestFood((next_x, next_y), food, walls)
        if dist is not None:
            # make the distance a number less than one otherwise the update͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
            # will diverge wildly͏󠄂͏️͏󠄌͏󠄎͏︄͏︊͏󠄅
